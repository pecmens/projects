以下是根据改进后的功能重新编写的详细文档，包含新增的“自动删除空行”功能及其实现方法。
...
5. **自动删除空行**：如果某一行的三个输入框（件数、重量、备注）均为空，则自动删除该行，但始终保留一个空白行用于数据输入。
...
#### 自动删除空行
- 定义`removeEmptyRow`函数，检查当前行的所有输入框是否为空。
- 如果某一行的三个输入框均为空，并且表格中至少还有一行数据，则删除该行。
- 删除行后递减`rowCount`变量，并更新总计值。
...
- 当检测到输入时，检查是否需要新增一行或删除当前行，并更新所有总计值。
...
### 5. 自动删除空行
- 如果某一行的三个输入框（件数、重量、备注）均为空，则该行会被自动删除。
- 表格始终保留至少一个空白行用于数据输入。
